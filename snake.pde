...
ArrayList<Integer> xmine = new ArrayList<Integer>();
ArrayList<Integer> ymine = new ArrayList<Integer>();
...
void draw () {
  ...
  //spawns any mines in mine arraylist
  for(int i = 0; i< xmine.size(); i++ ) {
        fill(0, 0, 0);
        rect(xmine.get(i) * bs, ymine.get(i) * bs, bs, bs);
  }
...
//    fill(0, 255, 0);
  ...
  if (!gameover) {
    if (frameCount % 5 == 0) {
      ...
      //opposite directional death needs works 
      if( x.size() > 2) {
         if(x.get(0) == x.get(1) && y.get(0) == y.get(1)){
           gameover = true;
         }
       }
      //Make sure that we do not run into mine 
      for(int i = 0; i< xmine.size(); i++ ) {
        if (x.get(0) == xmine.get(i) && y.get(0) == ymine.get(i) ){
          gameover = true;
        }
      }
      ...
      if(x.get(0) == applex && y.get(0) == appley) {
        ...
        minex = (int) random(0, w); 
        miney = (int) random(0, h);  
        xmine.add(minex);
        ymine.add(miney);
...
void keyPressed () {
  ...
  */
  ...
  int newdir = key == 's' ? 0 : (keyCode == DOWN ? 0 : (keyCode == UP ? 1 : (keyCode == RIGHT ? 2 : (keyCode == LEFT ? 3 :(key == 'w' ? 1 : (key == 'd' ? 2 : (key == 'a' ? 3 : -1)))))));
  if( x.size() > 1) {
         if(dir == 0 & newdir == 1 || dir == 1 & newdir == 0){
           gameover = true;
         }else if(dir == 1 & newdir == 0 || dir == 2 & newdir == 3){
           gameover = true;
         } 
   }
  if (newdir != -1) {
